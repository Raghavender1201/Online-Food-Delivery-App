...
public class RequestTraceFilter implements GlobalFilter {
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        if(isCorrelationIdPresent(headers)){
            logger.debug("CorrelationId is present: {}", filterUtility.getCorrelationId(headers));
        }else{
            String correlationId = generateCorrelationId();
            logger.debug("CorrelationId is not present, generating new CorrelationId: {}", correlationId);
            exchange = filterUtility.setCorrelationId(exchange, correlationId);
            logger.debug("New CorrelationId: {}", correlationId);
